...
//Code Here
let evenNumbers = mixedNumbers.filter(mixedNumber => mixedNumber % 2 == 0);
...
//Code Here
let postTaxPrices = prices.map(postTaxPrice => postTaxPrice * 1.07);
...
//Code Here
let totalPopulation = populations.reduce((poptotal, currpop, index, arr) => {return poptotal + currpop});
...
//Code Here
let myStrongest = monstersInYourPocket.filter(monsterInYourPocket => monsterInYourPocket.CP > 200);
...
//let orderTotals = orders.map(order => order * 1.07).filter(order => order.)
...
let bobsTotal = purchases.filter(purchase => [monsterInYourPocket].CP > 200);
